...
def test_serve_cli_validates_flags_then_hands_off_to_run_serve(monkeypatch, tmp_path, fixture_json,
    ...
    r = runner.invoke(app, ["asta", "serve", "--replay", str(FIXTURES / "asta_session_sample.jsonl"),
                            "--me", "0"])
    assert r.exit_code == ExitCode.OK, r.output
    assert called == {"mode": "replay", "me": "0"}
    assert "run " in r.output and SERVER_URL_DEFAULT in r.output
    bad = runner.invoke(app, ["asta", "serve", "--replay", str(tmp_path / "nope.jsonl")])
    assert bad.exit_code == ExitCode.USAGE


def test_serve_takes_no_host_or_port_flag(monkeypatch, tmp_path, fixture_json, mcp_fixture_json):
    """There is one address and no flag that can move it -- see the note over
    SERVE_HOST/SERVE_PORT. Typer answers an unknown option with exit 2, and
    that is the whole contract: nobody can start a second-port server by
    accident and then wonder why `asta adjust` wrote the file itself."""
    _ranked(monkeypatch, tmp_path, fixture_json, mcp_fixture_json)
    replay = str(FIXTURES / "asta_session_sample.jsonl")
    for flag in (["--port", "9000"], ["--host", "0.0.0.0"]):
        r = runner.invoke(app, ["asta", "serve", "--replay", replay, "--me", "0", *flag])
        assert r.exit_code == ExitCode.USAGE, (flag, r.exit_code, r.output)


def test_one_host_and_port_across_every_file_that_names_it():
    """The six places the address appears must agree, and only three of them
    are Python: .mcp.json is hand-written JSON and vite.config.ts is the
    dashboard's dev proxy, so nothing but this test compares them to the
    constant the server actually binds. A drift here is silent in production
    -- the MCP dials a dead port, `poe web-dev` proxies into nothing -- so it
    is a red test instead."""
    import re
    from urllib.parse import urlsplit

    from fantaclaude.commands.asta import SERVE_HOST, SERVE_PORT, SERVER_URL_DEFAULT
    repo_root = Path(__file__).resolve().parents[2]
    netloc = f"{SERVE_HOST}:{SERVE_PORT}"
    assert urlsplit(SERVER_URL_DEFAULT).netloc == netloc

    config = json.loads((repo_root / ".mcp.json").read_text(encoding="utf-8"))
    assert urlsplit(config["mcpServers"]["fantaclaude-asta"]["url"]).netloc == netloc

    vite = (repo_root / "web" / "vite.config.ts").read_text(encoding="utf-8")
    targets = dict(re.findall(r'"(/api|/ws)":[^\n]*?(?:target:\s*)?"(\w+://[^"]+)"', vite))
    assert targets == {"/api": f"http://{netloc}", "/ws": f"ws://{netloc}"}, vite
